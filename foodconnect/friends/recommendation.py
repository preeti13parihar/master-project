    ...
    def get_friend_recommendation(self, request):
        try:
            ...
            if len(top_reviews) > 0:
                similar_users = Reviews.objects.filter(Q(restaurant_id__in=top_reviews) &  ~Q(user_id=user_id) & Q(rating__range=(3,5)) ).values_list('user_id', flat=True)
                if len(list(similar_users)) > 0:
                    user_details = user_model.objects.all().filter(uuid__in=list(similar_users)).values()
                else:
                    user_details = user_model.objects.values()[:5]
            else:
                user_details = user_model.objects.values()[:5]
